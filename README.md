...
| --- | --- | --- | --- |-----|-----|
| Users | 48 Гб | 2 | 3585 | горизонтальное шардирование, шарды по диапазонам user_id (range)| master-slave репликация - по 2 реплики на сервер|
| Swipes | 128 Гб | 3470 | 3240 | горизонтальное шардирование, шарды по диапазонам  user_target_id (range)| master-slave репликация - по 2 реплики на сервер|
| Sessions | 7 Гб | 2 | 8300 | шарды по хэш слотам(Redis) | master-slave репликация - по 2 реплики на сервер|
| Messages | 7.5 Тб | 2300 | 2300 | горизонтальное шардирование, шарды по диапазонам  match_id (range)| master-slave репликация - по 2 реплики на сервер|
| Matches | 45 Гб | 115 | 115 | горизонтальное шардирование, шарды по диапазонам  first_user_id(range) | master-slave репликация - по 2 реплики на сервер|
| Photos | 140 Гб | 2 | 3585 | горизонтальное шардирование, шарды по диапазонам  user_id(range)| master-slave репликация - по 2 реплики на сервер |
...
#### Схема шардинга
Записи в `Users`, `Photos` будут принадлежать конкретному шарду, если конкректный id пользователя соответствующей записи принадлежит диапазону user_id данного шарда.\
Записи в `Swipes` будут принадлежать конкретному шарду, если конкректный id пользователя соответствующей записи принадлежит диапазону target_user_id данного шарда.\
В таблице `Swipes` есть записи дубликаты, для получения всех записей по target_user_id\
Записи в `Matches` будут принадлежать конкретному шарду, если конкректный id пользователя соответствующей записи принадлежит диапазону first_user_id данного шарда.\
В таблице `Matches` есть записи дубликаты, для получения всех записей по first_user_id\
Записи в `Messages` будут принадлежать конкретному шарду, если конкректный id мэтча соответствующей записи принадлежит диапазону match_id данного шарда.\
Для `Sessions` используем Redis, который имеет свой алгоритм распределения строк по ключа по разным узлам\
...
|----------| ---------- |
| Users    | user_id    |
| Swipes   | user_target_id    |
| Matches  | first_user_id |
| Messages | match_id |
| Photos   | user_id   | 
