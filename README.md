...
| Matches | 45 Гб | 115 | 115 | горизонтальное шардирование, шарды по диапазонам  match_id(range) | master-slave репликация - по 2 реплики на сервер|
...
#### Схема шардинга
Записи в Users, Swipes, Photos будут принадлежать конкретному шарду, если конкректный id пользователя соответствующей записи принадлежит диапазону\ user_id данного шарда.\
Записи в Messages, Matches будут принадлежать конкретному шарду, если конкректный id мэтча соответствующей записи принадлежит диапазону match_id данного\ шарда.\
